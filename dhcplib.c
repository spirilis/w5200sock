...
int dhcp_read_header(int sockfd, DHCPHeaderPreamble *preamble, uint16_t *ciaddr, uint16_t *yiaddr, uint16_t *siaddr, uint16_t *giaddr, uint16_t *chaddr)
{
	...
	if (wiznet_ntohs(scratch) != DHCP_MAGIC_COOKIE_0 ||
		...
		// Set a code about reading invalid magic cookie?
		return -1;
	}
...
int dhcp_read_option(int sockfd, uint8_t *option, uint8_t *len, void *buf)
{
	...
	*option = opthdr[0];
	*len = opthdr[1];
	//uartcli_print_str("dhcp_read_option opt = "); uartcli_print_int(opthdr[0]); uartcli_print_str(", len = "); uartcli_println_int(opthdr[1]);
...
int dhcp_send_dhcpdiscover(int sockfd)
{
	...
	//uartcli_print_str("Our MAC: ");
	//uartcli_printhex_word(ourmac[0]);  uartcli_printhex_word(ourmac[1]);  uartcli_printhex_word(ourmac[2]); uartcli_println_str(" ");
...
int dhcp_loop_configure(uint16_t *dnsaddr)
{
	...
	while (loopcount < DHCP_LOOP_COUNT_TIMEOUT && exitval == 1) {  // exitval=1 means keep looping, 0 means we're done
		switch (state) {
			case 0:
				// Send DHCPDISCOVER
				// uartcli_println_str("Send DHCPDISCOVER:");
			...
			case 1:
				// Check for DHCPOFFER
				// uartcli_println_str("Check for DHCPOFFER...");
				if (wiznet_recvsize(sockfd)) {
					// uartcli_println_str("Found packet:");
					...
					if (optcode == DHCP_OPTCODE_DHCP_MSGTYPE && optlen == 1 && scratch[0] == DHCP_MSGTYPE_DHCPOFFER) {
						// uartcli_println_str("Packet is DHCPOFFER");
							...
							switch (optcode) {
								...
								case DHCP_OPTCODE_ROUTER:
									if (optlen == 4) {
										giaddr[0] = wiznet_ntohs(scratch);
										giaddr[1] = wiznet_ntohs(scratch+2);
										// uartcli_print_str("gateway = "); uartcli_printhex_word(giaddr[0]); uartcli_printhex_word(giaddr[1]); uartcli_println_str(" ");
									}
									break;
								case DHCP_OPTCODE_DHCP_SERVER_IP:
									if (optlen == 4) {
										siaddr[0] = wiznet_ntohs(scratch);
										siaddr[1] = wiznet_ntohs(scratch+2);
										// uartcli_print_str("DHCP server = "); uartcli_printhex_word(siaddr[0]); uartcli_printhex_word(siaddr[1]); uartcli_println_str(" ");
									}
									break;
								case DHCP_OPTCODE_DOMAIN_SERVER:
									if (optlen >= 4) {  // Only extract the first DNS server.
										...
										// uartcli_print_str("DNS = "); uartcli_printhex_word(dns[0]); uartcli_printhex_word(dns[1]); uartcli_println_str(" ");
									}
							}
						...
						// uartcli_print_str("Packet is not DHCPOFFER: ");
						// uartcli_println_int(optcode);
					}
				}
			...
			case 2:  // Send DHCPREQUEST (officially requesting the lease)
				// uartcli_println_str("Send DHCPREQUEST:");
			...
			case 3:  // Receive DHCPACK (officially acknowledgement that the lease is ours)
				// uartcli_println_str("Check for DHCPACK...");
				if (wiznet_recvsize(sockfd)) {
					// uartcli_println_str("Found packet:");
					if ( (ret = wiznet_recvfrom(sockfd, NULL, 0, ipzero, &srcport, 0)) < 0 ) {
					...
					// uartcli_print_str("yiaddr_ack = "); uartcli_printhex_word(yiaddr_ack[0]); uartcli_printhex_word(yiaddr_ack[1]); uartcli_println_str(" ");
					// uartcli_print_str("siaddr_ack = "); uartcli_printhex_word(siaddr_ack[0]); uartcli_printhex_word(siaddr_ack[1]); uartcli_println_str(" ");
					// Check OP field; should be 0x02, else this isn't a server->client reply!
					if (pamb.op != 0x02) {
					...
					if (optcode == DHCP_OPTCODE_DHCP_MSGTYPE && optlen == 1 && scratch[0] == DHCP_MSGTYPE_DHCPACK) {
						// uartcli_println_str("Packet is DHCPACK");
						// Read all headers to extract info
						do {
							dhcp_read_option(sockfd, &optcode, &optlen, scratch);
						...
						wiznet_flush(sockfd, 10000, 1);  // Flush any remaining bytes (should be none), signal more may come.
						// uartcli_println_str("DHCPACK contents processed;");
						...
						if (yiaddr[0] != yiaddr_ack[0] || yiaddr[1] != yiaddr_ack[1] ||
							...
							// uartcli_println_str("Contents don't match up to DHCPOFFER");
							continue;
						}
						if (dns[0] != 0xFFFF && dns[1] != 0xFFFF) {
							// Is the user requesting that DNS be reported back?
							// uartcli_println_str("DNS was provided");
							if (dnsaddr != NULL) {
								dnsaddr[0] = dns[0];
								dnsaddr[1] = dns[1];
							}
						...
						// uartcli_println_str("Configuring WizNet IP settings");
						...
						wiznet_flush(sockfd, 10000, 1);
						// uartcli_print_str("Packet is not DHCPACK: ");
						// uartcli_println_int(optcode);
					}
				}
				break;  // If there is no data waiting, loop will just continue another round.
...
char *dhcp_strerror(int errno)
{
	...
	return (char *)dhcplib_errno_descriptions[errno];
}
